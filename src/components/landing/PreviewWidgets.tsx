    ...
    <div className="space-y-3">
      ...
      {Array.from({ length: 4 }, (_, row) => (
        <div key={row} className="flex items-center gap-2">
          <span
            className={cn(
              "size-7 rounded-lg",
              row === index % 4 ? "bg-signal-strong shadow-[0_0_20px_rgb(var(--signal-rgb)/0.45)]" : "bg-white/10",
            )}
          />
      ...
      <div className="grid grid-cols-5 gap-1.5 pt-2" aria-hidden="true">
        {Array.from({ length: 15 }, (_, cellIndex) => (
          <span
            key={cellIndex}
            className={cn("case-cell h-8 rounded", cellIndex % (index + 2) === 0 ? "bg-signal-strong/55" : "bg-white/8")}
...
function BotPreview() {
  return (
    <div className="space-y-2 rounded-lg border border-white/10 bg-black/28 p-3 shadow-[inset_0_0_28px_rgb(var(--signal-rgb)/0.04)]">
      {botLines.map((line, index) => (
        <div
          key={line}
          className={cn(
            "chat-pop w-fit max-w-[88%] rounded-lg px-3 py-2 text-xs",
            index === 1 ? "ml-auto bg-signal-strong text-slate-950" : "bg-white/10 text-slate-200",
          )}
    ...
    <div className="rounded-lg border border-white/10 bg-black/28 p-3">
        ...
        {pipelineLabels.map((item, index) => (
          <span
            ...
            className={cn(
              ...
              index === 1 ? "bg-signal-strong/80 text-slate-950" : "bg-white/8 text-slate-300",
            )}
          >
            {item}
          </span>
      ...
      {pipelineStatuses.map((status, index) => (
        ...
        >
          ...
          <span className={cn("rounded px-2 py-1.5 text-center", index === 0 ? "bg-signal-strong/80 text-slate-950" : "bg-signal-strong/16 text-signal-bright")}>
            {index + 2}
          </span>
        </div>
      ))}
    ...
    <div className="rounded-lg border border-white/10 bg-black/28 p-3">
      <div className="mb-3 grid h-20 grid-cols-5 gap-1.5 rounded-lg bg-gradient-to-br from-signal-strong/12 via-signal/8 to-signal-strong/16 p-2" aria-hidden="true">
        {Array.from({ length: 15 }, (_, index) => (
          <span
            key={index}
            className={cn("matrix-cell rounded-sm", index % 4 === 0 ? "bg-signal-strong/80" : index % 3 === 0 ? "bg-signal-strong/45" : "bg-white/10")}
