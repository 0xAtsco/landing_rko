    ...
    <SectionReveal id="timeline" className="relative px-4 py-20 sm:px-6">
      ...
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.78fr_1.22fr]">
        <div className="top-24 h-fit rounded-lg border border-white/10 bg-black/16 p-5 backdrop-blur-xl lg:sticky">
          ...
          <div className="mt-7 rounded-lg border border-signal/12 bg-[var(--surface-2)]/75 p-4">
            <div className="mb-3 flex justify-between font-mono text-[10px] uppercase tracking-[0.16em] text-slate-400">
              ...
              <span className="text-signal">результат</span>
            </div>
            <div className="grid grid-cols-8 gap-1.5" aria-hidden="true">
              {timelineSection.items.map((item, index) => (
                <span key={item.step} className={index === timelineSection.items.length - 1 ? "h-10 rounded-sm border border-signal/50 bg-signal-strong/45" : "h-10 rounded-sm border border-white/8 bg-white/[0.06]"} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="space-y-5">
            {timelineSection.items.map((item, index) => (
              ...
              >
                <span className={index % 2 === 0 ? "absolute left-0 top-5 grid size-10 place-items-center rounded-full border border-signal/35 bg-[var(--surface-2)] text-xs font-semibold text-signal-bright shadow-[0_0_22px_rgb(var(--signal-rgb)/0.24)] md:left-auto md:right-[-1.25rem]" : "absolute left-0 top-5 grid size-10 place-items-center rounded-full border border-signal/35 bg-[var(--surface-2)] text-xs font-semibold text-signal-bright shadow-[0_0_22px_rgb(var(--signal-rgb)/0.24)] md:left-[-1.25rem]"}>
                  {item.step}
                </span>
