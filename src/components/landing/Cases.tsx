...
export function Cases() {
  return (
    <SectionReveal id="cases" className="relative px-4 py-20 sm:px-6">
      <div aria-hidden="true" className="absolute left-0 top-24 h-72 w-full bg-[radial-gradient(circle_at_70%_35%,rgb(var(--signal-rgb)/0.14),transparent_34rem)]" />
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {cases.map((item, index) => (
            <article
              ...
              data-reveal
              data-hover-glow
              className="group motion-tilt-card relative flex min-h-[720px] flex-col overflow-hidden rounded-lg border border-white/10 bg-[var(--surface-2)]/82 p-4 shadow-[0_20px_70px_rgba(0,0,0,0.2)] transition motion-safe:hover:-translate-y-1 hover:border-signal/30 hover:shadow-[0_24px_90px_rgb(var(--signal-rgb)/0.12)] md:backdrop-blur-md"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-signal-strong/10 via-transparent to-signal-strong/14 opacity-75" />
              ...
              <div className="relative z-10 flex h-full flex-col">
                ...
                <div className="mb-5 overflow-hidden rounded-lg border border-white/10 bg-black/24 p-2 shadow-[inset_0_0_30px_rgb(var(--signal-rgb)/0.05)]">
                  {item.image.length > 0 ? (
                    <div className="relative overflow-hidden rounded-md">
                      <Image
                        ...
                        className="aspect-[16/10] w-full rounded-md object-cover shadow-[0_18px_70px_rgb(var(--signal-rgb)/0.12)] transition duration-500 group-hover:-translate-y-1 group-hover:scale-[1.015]"
                        loading="lazy"
                      />
                      <div className="case-preview-overlay absolute inset-x-3 bottom-3 rounded-md border border-signal/18 bg-[var(--surface-1)]/82 p-2 shadow-[0_12px_34px_rgba(0,0,0,0.28)]">
                        <div className="mb-2 flex items-center justify-between font-mono text-[9px] uppercase tracking-[0.14em] text-signal-bright">
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                          {[62, 88, 44, 76].map((value, barIndex) => (
                            <span key={value} className="h-1.5 overflow-hidden rounded-full bg-white/10">
                              <span className="block h-full origin-left rounded-full bg-signal-strong/80 motion-safe:animate-[quality-bar-in_.8s_ease-out_both]" style={{ transform: `scaleX(${value / 100})`, animationDelay: `${barIndex * 80}ms` }} />
                            </span>
                          ))}
                        </div>
                      </div>
                ...
                <span className="w-fit rounded-lg border border-signal/20 bg-signal/10 px-2.5 py-1 text-[11px] uppercase tracking-[0.16em] text-signal-bright">
                  {item.tag}
                </span>
...
function CaseLine({ label, text }: { label: string; text: string }) {
  return (
    <div className="rounded-lg border border-white/8 bg-white/[0.035] p-3">
      <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-signal/80">
        {label}
      </div>
  ...
  return (
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_0.9fr]">
        ...
        <div className="rounded-lg border border-signal/14 bg-signal/[0.055] p-2">
          <div className="mb-2 grid grid-cols-3 gap-1">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <span key={item} className="case-cell h-7 rounded bg-signal-strong/30" style={{ animationDelay: `${item * 0.05}s` }} />
            ))}
          </div>
...
function SalesMockup() {
  return (
    <div>
      ...
      <div className="mt-3 rounded-lg border border-signal/18 bg-signal-strong/10 p-2 text-xs text-signal-bright">
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {["Hook", "Script", "Post"].map((item, index) => (
          <div key={item} className="case-cell min-h-24 rounded-lg border border-white/8 bg-gradient-to-b from-signal-strong/18 to-signal-strong/12 p-2" style={{ animationDelay: `${index * 0.12}s` }}>
            <div className="mb-8 rounded bg-white/16 px-2 py-1 text-[10px] text-white">{item}</div>
            <div className="h-1.5 rounded bg-white/20" />
            <div className="mt-1.5 h-1.5 w-2/3 rounded bg-signal/35" />
          </div>
        ))}
      </div>
      <div className="mt-3 grid grid-cols-5 gap-1">
        {Array.from({ length: 15 }).map((_, index) => (
          <span key={index} className="h-2 rounded bg-white/10 odd:bg-signal-strong/45" />
        ))}
      </div>
    </div>
  );
...
function RkoMockup() {
  ...
  return (
    <div>
      ...
      <div className="relative mb-3 overflow-hidden rounded-lg border border-signal/14 bg-black/24 p-3">
        <svg aria-hidden="true" className="absolute inset-0 h-full w-full">
          <line x1="12%" y1="50%" x2="88%" y2="50%" stroke="var(--signal-bright)" strokeOpacity=".35" strokeWidth="1" strokeDasharray="6 8" className="signal-flow" />
        </svg>
        <div className="relative z-10 grid grid-cols-5 gap-2">
          {nodes.map((node, index) => (
            <div key={node} className="grid min-h-12 place-items-center rounded-lg border border-white/10 bg-[var(--surface-2)]/90 px-1 text-center text-[10px] text-slate-200 shadow-[0_0_20px_rgb(var(--signal-rgb)/0.08)]">
              {node}
              <span className={`mt-1 size-1.5 rounded-full ${index === 2 ? "bg-signal shadow-[0_0_14px_rgb(var(--signal-rgb)/0.9)]" : "bg-white/25"}`} />
            </div>
          ))}
        </div>
      </div>
      <div className="grid gap-2 sm:grid-cols-2">
        <div className="chat-pop rounded-lg bg-signal-strong px-3 py-2 text-xs text-slate-950">Бот: какой банк интересен?</div>
      ...
      <div className="overflow-hidden rounded-lg border border-white/10 bg-black/24">
        {["Лид", "Статус", "Ответственный", "Следующий шаг"].map((cell) => (
          <div key={cell} className="grid grid-cols-[1fr_76px] gap-2 border-b border-white/8 px-3 py-2 text-xs last:border-b-0">
            ...
            <span className="rounded bg-signal-strong/16 px-2 py-0.5 text-center text-signal-bright">ok</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-[1fr_72px] gap-3">
        <div className="rounded-lg border border-white/8 bg-white/[0.055] p-3">
          <div className="mb-3 h-3 w-24 rounded bg-signal/45" />
          <div className="space-y-2">
            <div className="h-2 rounded bg-white/18" />
            <div className="h-2 w-4/5 rounded bg-white/12" />
            <div className="h-8 rounded bg-signal-strong/20" />
          </div>
        </div>
        <div className="rounded-lg border border-signal/14 bg-black/24 p-2">
          <div className="mx-auto mb-2 h-14 w-8 rounded border border-white/10 bg-signal-strong/10" />
          <div className="h-2 rounded bg-white/18" />
          <div className="mt-1 h-2 rounded bg-signal/35" />
        </div>
      </div>
    </div>
  );
